...
def account():
    ...
    if form.validate_on_submit():
        ...
        form.picture_url.data = current_user.picture_url
        listings = User.query.filter_by(id=current_user.id).first().listings
    
    return render_template('account.html', title='Account', form=form, listings=listings)
